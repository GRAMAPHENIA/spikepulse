...
class SpikepulseApp {
    ...
    setupGameEventListeners() {
        if (!this.gameEngine || !this.gameEngine.eventBus) {
            console.warn('[Spikepulse] GameEngine o EventBus no disponible');
            return;
        }

        // Escuchar cambios de estado para actualizar UI
        this.gameEngine.eventBus.on('state:change', (data) => {
            console.log('[Spikepulse] Cambio de estado detectado:', data);
            this.handleStateChange(data);
        });

        // Escuchar eventos de UI del motor
        this.gameEngine.eventBus.on('ui:show-menu', () => {
            this.showMenuScreen();
        });

        this.gameEngine.eventBus.on('ui:hide-menu', () => {
            this.hideMenuScreen();
        });

        this.gameEngine.eventBus.on('ui:show-game', () => {
            this.showGameScreen();
        });

        this.gameEngine.eventBus.on('ui:show-pause', () => {
            this.showPauseOverlay();
        });

        this.gameEngine.eventBus.on('ui:hide-pause', () => {
            this.hidePauseOverlay();
        });

        this.gameEngine.eventBus.on('ui:show-game-over', () => {
            this.showGameOverScreen();
        });

        this.gameEngine.eventBus.on('ui:hide-game-over', () => {
            this.hideGameOverScreen();
        });

        // Escuchar eventos de error
        this.gameEngine.eventBus.on('engine:error', (data) => {
            console.error('[Spikepulse] Error del motor:', data);
            this.showError('Error en el motor del juego');
        });

        console.log('[Spikepulse] Listeners de eventos del juego configurados');
    }
    ...
    handleStateChange(data) {
        const { from, to } = data;
        console.log(`[Spikepulse] Transición de estado: ${from} -> ${to}`);
        ...
        switch (to) {
            ...
            case 'playing':
                ...
                break;
            case 'paused':
                this.showGameScreen(); // Mantener juego visible
                this.showPauseOverlay();
                break;
            case 'gameOver':
                this.showGameOverScreen();
                break;
            default:
                console.warn(`[Spikepulse] Estado desconocido: ${to}`);
        }

        // Anunciar cambio a lectores de pantalla
        this.announceStateChange(to);
    }
